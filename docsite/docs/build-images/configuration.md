...
If the build directory is the root of the repository, you pass the build path as `.`. If the build directory is some folder of the repo, just pass the name of the folder. Do not pass the preceding "/". For example `build/directory`
...
When connecting a build to a deployment, the default behavior is for the deployment to inherit the registry configuration from the build.
...
The Dockerfile may make use of [build args](https://docs.docker.com/engine/reference/builder/#arg). Build args can be passed using the gui by navigating to the `Build Args` tab in the config. They are passed in the menu just like they would in a .env file:
