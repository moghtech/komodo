...
- **Build images** — Build application source into images. Supports AWS EC2 spot instances for scalable build capacity.
- **Run automation** — Manage repos on servers, execute scripts, and orchestrate multi-step workflows with Procedures and Actions.
- **Manage configuration** — shared variables and secrets with interpolation across all resources.
...
| **Periphery** | Small, stateless agent running on each connected server. Called by Core to perform actions, report system usage, and stream container logs. Can connect via outbound or inbound connections — whichever suits your network. |
