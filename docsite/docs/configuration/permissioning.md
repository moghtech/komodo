...
Core can also control who is allowed to register in the first place:
...
- `KOMODO_DISABLE_LOCAL_USER_REGISTRATION=true`
  - blocks local username/password signups while leaving external login registration available
- `KOMODO_DISABLE_OIDC_USER_REGISTRATION=true`
  - blocks new-user registration through OIDC while leaving local signups unchanged
