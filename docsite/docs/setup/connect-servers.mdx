...
1.  **Create an Onboarding Key** via the Komodo Core API / UI (they can also be reused).
2.  **Install the Periphery agent** on the server or VM.
...
Edit the `/etc/komodo/periphery.config.toml` config file and modify as required.


Enable and start the Periphery service (autostart on reboot).

```
systemctl enable periphery
systemctl start periphery  # restart if you edited the periphery.config.toml
```
